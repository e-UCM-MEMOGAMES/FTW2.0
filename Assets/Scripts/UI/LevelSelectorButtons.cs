...
public class LevelSelectorButtons : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        int[] beatenMapsPrevLevel = new int[levelButtons.Length];
        int[] playedMapsPrevLevel = new int[levelButtons.Length];
        ...
        for (int levelNumber = 0; levelNumber < levelButtons.Length; levelNumber++)
        {
            for (int mapNumber = 0; mapNumber < mapButtons.Length; mapNumber++)
            {
                ...
                prevLvl = Defs.GetLevelKey(levelNumber + 1, mapNumber);
                currLvl = Defs.GetLevelKey(levelNumber + 1, mapNumber + 1);
                ...
                {
                    playedMapsPrevLevel[levelNumber]++;
                }
                if (mapUnlocked && PlayerPrefs.GetInt(currLvl) >= minStarsPerLevel)
                {
                    beatenMapsPrevLevel[levelNumber]++;
                }
            }
            bool levelUnlocked = levelNumber == 0 || (playedMapsPrevLevel[levelNumber - 1] >=mapButtons.Length && beatenMapsPrevLevel[levelNumber - 1] >= neededLevelsWithMinStars);
